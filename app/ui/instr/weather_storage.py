...
class WeatherStatistics(WeatherStorage):
    ...
    def _get_raw_data(self, city, start_date, end_date):
        ...
        raw_lines = [
            set(self.db_driver.query.filter(condition).all())
    ...
    def prec_statistic(self):
        ...
        for stat in prec_stat:
            prec_stat[stat] = str(round((prec_stat[stat] / len(prec_values) * 100), 0))

        prec_descr_count = Counter(
            [elem.prec_desc for elem in self.raw_data if elem.prec_desc != " "]
        )
    ...
    def wind_statistic(self):
        ...
        wind_directs = Counter(
            [elem.wind_direc for elem in self.raw_data if elem.prec_desc != " "]
        )
        wind_stat = {

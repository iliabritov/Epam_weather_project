...
def load_input_data(
    path, cities_path, file_ext, url_path=None, headers_path=None, form_data_path=None
):
        ...
        for city in input_data["cities"]:
            ...
            for year in range(start_date.year, end_date.year + 1):
                ...
                input_data["form_data"][
                    "a_date1"
                ] = f"{a_date1[2]}.{a_date1[1]}.{str(year)}"
                input_data["form_data"][
                    "a_date2"
                ] = f"{a_date2[2]}.{a_date2[1]}.{str(year)}"
                gz_dataset = load_rp5_dataset(
                    s,
...
def load_wwo_weather_datasets(input_data, start_date, end_date):
    ...
    for city in input_data["cities"]:
        ...
        while True:
            ...
            url = input_data["request_url"] + "&".join(
                [
                    key + "=" + input_data["form_data"][key]
                    for key in input_data["form_data"]
                ]
            )
            ...
            date_diff = calendar.monthrange(
                local_start_date.year, local_start_date.month
            )[1]
